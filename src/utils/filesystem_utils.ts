...
export const buildSessionDirs = () => {
	...
	let dirs = [
		path.dirname(finalMergedDocOutputPath),
		inputFileDirPath,
		tmpDirPath,
	];
	dirs.forEach((e) => {
...
export const extractDataFile = async (
...
export const clearDirectoryAndKeepDotFiles = (directoryPath: string) => {
	devLogger.info(
		`Removing all non dot files and folders from ${directoryPath}`
	);
